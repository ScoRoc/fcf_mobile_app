...
class EventsSubScreen extends React.Component {
  ...
  createMonthSection = month => ({
    title: month.month,
    data: month.events.filter(this.isActiveType),
  });
  ...
  filterEventTypes = type => {
    ...
    this.setState({
      eventTypes: this.toggleTypeInArray(type, eventTypes),
      removedTypes: this.toggleTypeInArray(type, removedTypes),
    });
  };
      ...
      if (userId !== userIdFromRedux) {
        ...
        const updateMonthEvents = (month, updatedEvent) => {
          const updatedEvents = month.events.map(mappedEvent => {
            return mappedEvent._id === updatedEvent._id ? updatedEvent : mappedEvent;
          });
